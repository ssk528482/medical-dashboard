...
function renderIntelligenceAlerts(containerId) {
  // Accept both a string ID and a DOM element
  ...
  // Async augment: smart flashcard load warning
  augmentAlertsWithCardLoad(container);
}

// ─── Smart Card-Load Warning ───────────────────────────────────
// Appends a warning banner when due flashcards exceed the daily comfort zone (40).
async function augmentAlertsWithCardLoad(containerOrId) {
  ...
  if (container.querySelector('[data-card-load-alert]')) return;
  try {
    let { data } = await fetchDueCards(today());
    let count = data?.length || 0;
    if (count < 40) return; // comfortable load — no warning

    let severity = count >= 80 ? 'high' : 'medium';
    let colors = {
      high:   { bg: "#450a0a", border: "#ef4444", text: "#fca5a5" },
      medium: { bg: "#451a03", border: "#f59e0b", text: "#fcd34d" }
    };
    let c = colors[severity];
    let banner = document.createElement('div');
    banner.setAttribute('data-card-load-alert', '1');
    banner.innerHTML = `
      <div style="background:${c.bg};border:1px solid ${c.border};border-radius:12px;padding:12px 14px;margin-bottom:8px;display:flex;gap:10px;align-items:flex-start;">
        <span style="font-size:20px;flex-shrink:0;">🃏</span>
        <div>
          <div style="font-size:13px;font-weight:700;color:${c.text};margin-bottom:2px;">${count} flashcards due today</div>
          <div style="font-size:12px;color:${c.text};opacity:0.85;">${count >= 80 ? 'Very heavy load — consider splitting across 2 sessions.' : 'High card load — pace yourself and use keyboard shortcuts (Space/1-4).'} <a href="review.html" style="color:${c.text};font-weight:700;">Review now →</a></div>
        </div>
      </div>`;
