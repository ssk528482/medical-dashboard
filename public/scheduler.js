...
function subjectPriority(subjectName) {
  ...
  // Phase completion boost: behind on phase 2/3 = higher priority
  let phase = detectPhaseStatus(subjectName);
  let phaseBoost = phase.phase1 ? 0 : 5;
  phaseBoost += phase.phase2 ? 0 : (examProximityFactor() * 15);

  // Exam proximity: boost weak subjects more aggressively near exam
  let proximityMultiplier = 1 + examProximityFactor() * 0.5;

  // Consecutive day penalty (rotation limiter): reduce if same subject was today's plan
  let consecutivePenalty = 0;
  if (studyData.dailyPlan && studyData.dailyPlan.study.subject === subjectName) {
    // Check if yesterday also had this subject
    let yesterday = addDays(today(), -1);
    if (studyData.dailyHistory?.[yesterday]?.subject === subjectName) {
...
function getBurnoutAdjustment() {
  ...
  // If burnout > 50, reduce recommended hours by up to 30%
...
function generatePlan() {
  ...
  // Dynamic split: if heavy overdue, boost revision allocation
...
function submitEvening() {
  ...
  if (document.getElementById("qbankDone").checked) {
    ...
    if (studyData.subjects[subjectName]?.topics[topicIndex]) {
      ...
      // Adjust difficulty factor based on qbank accuracy
      if (total > 0) {
        let acc = correct / total;
        // Low accuracy → reduce EF (harder topic), high accuracy → increase
        let qualityScore = Math.round(acc * 5);
  ...
  if (!studyData.dailyHistory[todayDate]) {
    studyData.dailyHistory[todayDate] = { study: false, qbank: false, revision: false };
  }
  if (document.getElementById("studyDone").checked) studyData.dailyHistory[todayDate].study = true;
  if (document.getElementById("qbankDone").checked) studyData.dailyHistory[todayDate].qbank = true;
  if (revisionCheckboxes.length > 0) studyData.dailyHistory[todayDate].revision = true;
