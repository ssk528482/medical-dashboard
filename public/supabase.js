...
// Singleton — ensures only one GoTrueClient exists per browser context.
// router.js (which loads first) may have already created this instance.
if (!window._supabaseSingleton) {
  window._supabaseSingleton = supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
}
const supabaseClient = window._supabaseSingleton;
