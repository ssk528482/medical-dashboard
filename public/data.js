...
// ── Session-scoped cache clear ────────────────────────────────────────────
// sessionStorage is wiped on every page refresh and when a new tab is opened,
// but survives same-session page navigations (index → analytics → browse…).
// This means:
//   • Refresh / new tab → stale localStorage cleared → cloud is authoritative
//   • Page navigation within the same session → keep the clean cached copy
// Auth tokens (sb-* keys) are never touched.
(function () {
  if (!sessionStorage.getItem('_appSession')) {
    localStorage.removeItem('studyData');
    localStorage.removeItem('_sbIds');
    sessionStorage.setItem('_appSession', '1');
  }
})();
...
function migrateData(data) {
  ...
  if (data.version < 4) {
    ...
    Object.values(data.subjects || {}).forEach(subject => {
      if (subject.topics !== undefined) {
        ...
        // Only wrap in "General" if there are actual topics to migrate
  ...
  // v5 → v6: remove spurious empty "General" units created by v4 migration on empty topics
  if (data.version < 6) {
    Object.values(data.subjects || {}).forEach(subject => {
      if (!subject.units) return;
      const before = subject.units.length;
      subject.units = subject.units.filter(u => !(u.name === 'General' && u.chapters.length === 0));
      // If pointer now points out-of-bounds, reset it
      if (subject.units.length < before) {
        let ui = Math.min(subject.pointer?.unit || 0, Math.max(0, subject.units.length - 1));
        subject.pointer = { unit: ui, chapter: 0 };
      }
    });
    data.version = 6;
  }
